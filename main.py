...
from fastapi import FastAPI, File, Form, WebSocket, Depends
...
from model.db import Base, get_db, Engine, UserReport, UserReportTable
...
async def submit_issue(user_content: str = Form(...), db=Depends(get_db)):
    ...
    ticket_info_data = WorkOrderNumber(
        ...
        assistingUnit=ticket_info.get("assistingUnit"),
    ...
    try:
        ...
        return {'message': '问题已提交并通知成功', 'notification': notification, "severityLevel": ticket_info.get("severityLevel")}
    except Exception as e:
    ...
    try:
        ...
        for wo in work_orders:
            ...
            result.append({
                ...
                "category": "紧急类" if urgency_level == "urgent" else ("快速处理类" if urgency_level == "quick" else "普通类"),
...
async def get_solution(work_order_content: str = Form(None)):
    """
    获取解决方案
...
@app.post("/get-judge/")
async def get_judge(user_content: str, work_order_number: str):
    """
    获取工单评分
    :param work_order_number:
    :param user_content:用户回访内容
    :return:
    """
    ...
    data = {
        ...
        "messages": [
            {
                "content": "工单评价." + user_content + f'工单信息：{form_info}。+ 返回的json数据，键用英文表示，值用中文表示',
                "role": "user"
            }
        ]
    }
